...
<div class="wrapper ff-layout">
    <div class="container pb50">
        <div class="link-source" >
            ...
            <span>></span>
            <a href="https://mall.fengjr.com/benefits" target="_blank">购实惠</a>
            <span>></span>
            <span class="product-title">${goods.name}</span>
        </div>
        <div class="prodIntro clearfix" data-product-id="8C0D4444-86F2-422B-9835-F31C92D51307" data-event-id="">
            <div class="left">
                <div class="p-img"><img src="${ctx}/${goods.image}" alt="${goods.name}" /></div>
            </div>
            <div class="prodInfo"><p class="title">${goods.name}</p>
                <p class="desc">${goods.subhead}</p>
                <div class="price-info"><span>会员价</span> <span class="price orange">${goods.point}</span> <span class="unit">积分</span>
                [#if currentUser?? && currentUserPoint??]
                    <p class="my"><span>我的积分</span> <span class="balance">${currentUserPoint.available()}</span></p></div>
                [/#if]
                ...
                <p class="mt20"><span>数量</span> <span class="tb-stock ml40">
                    [#--<button class="subtract" title="减1">-</button>
                    <input style="ime-mode:disabled" class="bug-count" type="text" value="1" maxlength="8" data-max-count="title=&quot;请输入购买量&quot;" />
                    <button class="add" title="加1">+ </button>
                ...
                <div class="btn-group">
                    ...
                    <span class="orange focus-tip-detail"></span>
                </div>
            </div>
        </div>
