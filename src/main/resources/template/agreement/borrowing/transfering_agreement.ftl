...
<body>
	...
	<div style="text-align:right;">协议编号：<span>HSJR-${((borrowing.lendingDate)?string("yyyyMMdd"))!"XXXXXXXX"}-${(borrowing.id)!"XXXX"}</span></div>
	<div>借款人（融资人）：<span>${(borrower.loginName)!"XXX"}</span></div>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size: 12px; line-height: 36px;">
	    ...
	    <tr>
	        ...
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">${(borrowing.amount?string("currency"))!"￥XXXX.XX"}</td>
	    </tr>
	    <tr>
	        <td align="right" style="border-color: #ccc; border-style: solid; padding-right: 10px; border-width: 0px 1px 1px;">还款本息（人民币）：</td>
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">${(borrowing.repaymentAmount?string("currency"))!"￥XXXX.XX"}</td>
	    </tr>
	    ...
	    [#if borrowing.repaymentMethod?? && borrowing.repaymentMethod != "last_period_capital_plus_interest"]
		    <tr>
		        ...
		        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">每月${(borrowing.lendingDate?string("dd"))!"XX"}日（24：00前，节假日不顺延）</td>
		    </tr>
		[#else]
		    <tr>
		        <td align="right" style="border-color: #ccc; border-style: solid; padding-right: 10px; border-width: 0px 1px 1px;">还款日：</td>
			...
			[#list repayments as repayment]
				[#if repayment.state == "repaid"]
                    <tr>
                        ...
                        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0px;">（实际）${repayment.repaymentAmount?string("currency")}</td>
                    </tr>
				[#else]
                    <tr>
                        ...
                        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0px;">${(repayment.repaymentAmount)?string("currency")}</td>
                    </tr>
				[/#if]
			[/#list]
		[#else]
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size: 12px; line-height: 36px;">
	    ...
	    [#if recoveries != null && recoveries?size gt 0]
	        [#list recoveries as recovery]
	        	...
	        	[#else]
	        	<tr>
	            	<td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0px 1px 1px;">${secrecy("fullName",recovery.investor.username)!recovery.investor.corpName}</td>
	...
	<p class="p" style="text-indent:2em;">第三条本合同项下的借款币种为人民币。借款人借款金额为（${(borrowing.amount?string("currency"))!"￥XXXX.XX"}）借款期限：${(borrowing.lendingDate?string("yyyy年MM月dd日"))!"XXXX年XX月XX日"}至${(repaymentEndDate?string("yyyy年MM月dd日"))!"XXXX年XX月XX日"}。</p>
