...
<html lang="zh-CN">
<head>
	<title>${setting.basic.siteName}-华善金融借款协议</title>
	<style type="text/css">
		body {
			font-family: SimSun;
			font-size: 12px;
...
<body>
	...
	<p style="text-align:right;">协议编号：<span>HSJR-${((borrowing.lendingDate)?string("yyyyMMdd"))!"XXXXXXXX"}-${(borrowing.id)!"XXXX"}</span></p>
	<br/>
	<p>甲方（投资人）：</p>
	<br/>
	<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size: 12px; line-height: 36px;">
	    <tr>
	        <td align="left" style="border: 1px solid #ccc; padding-left: 10px; border-width: 1px 1px 1px 1px;">投资人</td>
	        <td align="left" style="border: 1px solid #ccc; padding-left: 10px; border-width: 1px 1px 1px 0px;">身份证号</td>
	    ...
	    [#if investments != null && investments?size gt 0]
            ...
            [#else]--]
                <tr>
                    <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px;  border-width: 0px 1px 1px 1px;">${secrecy("fullName",investment.investorLoginName)}</td>
                    <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0px 1px 1px 0;">
						[#if investment.investorCorp?? && investment.investorCorp]
                            <p>${secrecy("fullName",investment.investorCorporationIdCard)!"XXXXXX XXXXXXXX XXXX"}</p>
						[#else]
                            <p>${secrecy("idNo",investment.investorIdNo)!"XXXXXX XXXXXXXX XXXX"}</p>
						[/#if]
                    </td>
                    ...
                    <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">${borrowing.period}${borrowing.periodUnitDes}</td>
	    ...
	    [#else]
        	<tr>
            	...
            	<td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0px 1px 1px;">XXXXXXXX</td>
	...
	<br/>
		[#if borrower.corp?? && borrower.corp]
			<p>乙方（借款人）：<span>${(borrowerCorporation.corpName)!"XXX"}</span></p>
			<p>营业执照 &nbsp;：<span></span>${(borrowerCorporation.corpLicenseNo)!"XXXXXX XXXXXXXX XXXX"}</p>
		[#else]
			<p>乙方（借款人）：<span>${(borrowerInfo.realName)!"XXX"}</span></p>
			<p>身份证号 &nbsp;：<span></span>${(borrowerInfo.idNo)!"XXXXXX XXXXXXXX XXXX"}</p>
		[/#if]
	<br/>
	...
	<p class="p" style="text-indent:2em;">为此，经各方协商一致，于${(borrowing.lendingDate?string("yyyy年MM月dd日"))!"XXXX年XX月XX日"}签订如下协议，以共同遵照履行：</p>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size: 12px; line-height: 36px;">
	    ...
	    <tr>
	        ...
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">${(borrowing.amount?string("currency"))!"￥XXXX.XX"}&nbsp;&nbsp;大写:${numToRMB(borrowing.amount)!"￥XXXX.XX"}</td>
	    </tr>
	    <tr>
	        ...
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">
				...
				[#else]
					${(repayment.capitalInterest+repayment.getTotalOverdueInterest())?string("currency")!"￥XXXX.XX"}&nbsp;&nbsp;大写:${numToRMB(repayment.capitalInterest+repayment.getTotalOverdueInterest())!"￥XXXX.XX"}
				[/#if]--]
				${(borrowing.repaymentAmount)?string("currency")!"￥XXXX.XX"}&nbsp;&nbsp;大写:${numToRMB(borrowing.repaymentAmount)!"￥XXXX.XX"}
			</td>
	    </tr>
	    <tr>
	        <td align="right" style="border-color: #ccc; border-style: solid; padding-right: 10px; border-width: 0px 1px 1px;">平台服务费（借款）：</td>
            <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">${borrowing.repaymentFee?string("currency")!"￥XX.XX"}  &nbsp;&nbsp;支付方式为：${borrowing.repaymentFeeMethodDes}</td>
	    </tr>
	    <tr>
	        <td align="right" style="border-color: #ccc; border-style: solid; padding-right: 10px; border-width: 0px 1px 1px;">借款保证金：</td>
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">￥0.00&nbsp;&nbsp;支付方式为：放款时一次性扣除</td>
	    </tr>
	    [#if borrowing.periodUnit??]
		    <tr>
		        <td align="right" style="border-color: #ccc; border-style: solid; padding-right: 10px; border-width: 0px 1px 1px;">还款期数（${borrowing.periodUnitDes}）：</td>
	    ...
	    [#if borrowing.repaymentMethod?? && borrowing.repaymentMethod != "last_period_capital_plus_interest"]
		    <tr>
		        <td align="right" style="border-color: #ccc; border-style: solid; padding-right: 10px; border-width: 0px 1px 1px;">还款日：</td>
		        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">自${(borrowing.lendingDate?string("yyyy年MM月dd日"))!"XXXX年XX月XX日"}起，每月${(borrowing.lendingDate?string("dd"))!"XX"}日（24:00前，节假日不顺延）。如该自然月无对应日，则为该月最后一日。</td>
		    </tr>
		[#else]
		    <tr>
		        <td align="right" style="border-color: #ccc; border-style: solid; padding-right: 10px; border-width: 0px 1px 1px;">还款日：</td>
		        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">每月XX日（24：00前，节假日不顺延）</td>
		    </tr>
	    [/#if]
	    <tr>
	        <td align="right" style="border-color: #ccc; border-style: solid; padding-right: 10px; border-width: 0px 1px 1px;">还款日期：</td>
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">${(borrowing.lendingDate?string("yyyy年MM月dd日"))!"XXXX年XX月XX日"}至${(repaymentEndDate?string("yyyy年MM月dd日"))!"XXXX年XX月XX日"}止</td>
	    </tr>
	    <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size: 12px; line-height: 36px;">
	    <tr>
	        ...
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 1px 1px 1px 0px;">月偿还本金（元）</td>
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 1px 1px 1px 0px;">月偿还利息（元）</td>
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 1px 1px 1px 0px;">月付平台服务费（元）</td>
	        <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 1px 1px 1px 0px;">月偿偿还总额（元）</td>
	    </tr>
        ...
        [#else]
		    <tr>
                <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">yyyy年MM月dd日</td>
                <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">￥XXXX.XX</td>
                <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">￥XXXX.XX</td>
                <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">￥XXXX.XX</td>
                <td align="left" style="border-color: #ccc; border-style: solid; padding-left: 10px; border-width: 0 1px 1px 0;">￥XXXX.XX</td>
	...
	<br/>
	<p class="p" style="text-indent:2em;">1.2借款用途</p>
	...
	<p class="p" style="text-indent:2em;">1.3.1 本协议项下的借款币种为人民币。乙方借款金额为（${(borrowing.amount?string("currency"))!"￥XXXX.XX"}）借款期限：${(borrowing.lendingDate?string("yyyy年MM月dd日"))!"XXXX年XX月XX日"}至${(repaymentEndDate?string("yyyy年MM月dd日"))!"XXXX年XX月XX日"}。</p>
	...
	<p class="p" style="text-indent:2em;">1.3.3 在以下条件完全得到满足的情况下，丙方将给存管银行发出放款指令（即放款）：
丙方要求乙方提供的文件、资料，乙方已全部提供，并且其所载明的情况没有发生变化，该等文件、资料持续有效，或者乙方已就发生的变化作出令丙方满意的解释和说明；本协议项下借款本金自借款项目满标计息时视为借款成功，本电子协议自动生成，各方当事人可自行登录华善金融平台账号查看、下载、打印。由于协议涉及所有甲方信息，为了保护其隐私，各方当事人所查看的本电子协议，华善金融平台将屏蔽掉其他人的部分或全部信息，甲方，乙方表示认可并不持异议。乙方应从借款成功日起计付利息。如投标时间内，乙方的需求资金在规定期限内没有筹集满，华善金融平台宣布流标则协议不生效，在此期间不产生利息。
</p>
	...
	<h4>第三条 还款方式</h4>
	<p class="p" style="text-indent:2em;">3.1 经协商一致，甲方同意乙方按下列约定偿还借款本息。具体还款方式以本协议第一条确定的为准。</p>
	...
	<h4>乙方（盖章）：</h4>
	<br/>
	<h4>法定代表人/授权委托人（签名、捺印）：</h4>
	<br/>
	<h4>丙方（盖章）：上海华善金融信息服务有限公司</h4>
	<br/>
	<h4>签署日期：&nbsp;&nbsp;&nbsp;&nbsp;年&nbsp;&nbsp;&nbsp;&nbsp;月&nbsp;&nbsp;&nbsp;&nbsp;日</h4>
</body>
</html>
