<div class="mui-content" xmlns="http://www.w3.org/1999/html">
    ...
    [#if transferId==currentUser.id]
    <div class="instr">甲方（债权出让方）：${userNameTransfer}</div>
    <div class="instr">身份证号/机构代码 ：${userIdNoTransfer}</div>
    [#else ]
        <div class="instr">甲方（债权出让方）：${secrecy("fullName",userNameTransfer)}</div>
        <div class="instr">身份证号/机构代码 ：[#if userTransferType=="GENERAL"]${secrecy("idNo",userIdNoTransfer)}[#else ]${secrecy("corpLicenseNo",userIdNoTransfer)}[/#if]</div>
    [/#if]
    <br>
[#if investId==currentUser.id]
    <div class="instr">乙方（债权受让人）：[#if investType=="GENERAL"]${investRealName}[#else ] ${investCorpName}[/#if]</div>
    <div class="instr">身份证号/机构代码：[#if investType=="GENERAL"]${investIdNo}[#else ] ${investCorpLicenseNo}[/#if]</div>
[#else ]
    <div class="instr">乙方（债权受让人）：[#if investType=="GENERAL"]${secrecy("fullName",investRealName)}[#else ] ${secrecy("corpName",investCorpName)}[/#if]</div>
    <div class="instr">身份证号/机构代码：[#if investType=="GENERAL"]${secrecy("idNo",investIdNo)}[#else ] ${secrecy("corpLicenseNo",investCorpLicenseNo)}[/#if]</div>
[/#if]
</br>
</br>
    <div class="instr">
    ...
    <p>2.1 甲乙双方约定标的债权的转让价款为￥${transferPrice}元（“债权转让价款”）。转让管理费用为￥${outFee}元。</p>
