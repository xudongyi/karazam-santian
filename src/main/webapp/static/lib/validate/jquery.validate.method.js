...
jQuery.validator.addMethod("lessThan", function(value, element, param) {
	...
	switch(typeof param) {
		...
		case "string": {
			var $target = $(param);
			if($target && $target.val() != "") {
				return value.replace(/[^\d]/g, '') * 1 < $target.val().replace(/[^\d]/g, '') * 1;
			} else {
				return true;
			}
		}
...
jQuery.validator.addMethod("areacity", function(value, element) {
	var $select = $(element).nextAll("select");
	return this.optional(element) || ($select.length > 1 && $select.last()[0].selectedIndex > 0);
}, $.validator.format("请选择城市"));