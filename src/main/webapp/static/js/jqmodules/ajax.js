...
layui.define(['jquery', 'form', 'layer'], function(exports) {
    ...
    ajax.prototype.init = function() {
        ...
        //对有.ajax-delete的元素绑定单击ajax事件
        $(".ajax-delete:not([ajax-bind])").each(function() {
            _this.bind($(this));
            _this.options.data._method = 'DELETE';
            _this.options.confirm = true;
            $(this).on('click', { ajax: _this }, _this.click);
        });
    ...
    ajax.prototype.callBack = function(ret, options) {
        ...
        if ("success" == ret.status) {
            if ((undefined == ret.data.url) || (null == ret.data.url) || ("" == ret.data.url)) {
                layer.closeAll();
                layer.msg(ret.message);
                location.reload();
                return;
            } else if ('reload' == ret.data.url) {
                location.reload();
