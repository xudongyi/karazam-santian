...
$(function () {
    dataLoad(state);
    $("#borrowing-apply").hide();
    $("#borrowing-apply-content").show();
    $("#borrowing").show();
    $("#borrowing-content").hide();
    $("#dateRange ul li").click(function(){
        $(this).addClass("cur").siblings().removeClass("cur");
        setDateRange($(this).text());
        if (state=='APPLY'){
            $("#borrowing-apply").show();
            $("#borrowing-apply-content").show();
            $("#borrowing").hide();
            $("#borrowing-content").hide();
        }else {
            $("#borrowing-apply").hide();
            $("#borrowing-apply-content").hide();
            $("#borrowing").show();
            $("#borrowing-content").show();
        }
        dataLoad(state);
    });
...
function dataLoad(state) {
    ...
    $.getJSON(ctx + '/uc/borrowing/data/'+state, params, function (res) {

        //使用方式跟独立组件完全一样
        var tpl;
        if (state=='APPLY'){
            tpl = document.getElementById("dataTpl-borrowing-apply");
        }else {
            tpl = document.getElementById("dataTpl-borrowing");
        }
        ...
        laypage({
            ...
            , jump: function (e, first) { //触发分页后的回调
                if (!first) {
                    $.getJSON(ctx + '/uc/borrowing/data/' + state, $.extend({
                        page: e.curr,//当前页
                        rows: res.pageSize
