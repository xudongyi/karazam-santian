$(function () {
    console.log(dizhi);
    if($("#type").val()!='TEXT'){
        $("#logoTr").html("<td>LOGO：</td><td><div class='input-group'><input id='logo' name='logo' class='easyui-textbox' value='' data-options=\"width:180,required:\'required\'\"/><label class='input-group-addon' onclick=\"uploadBanner('logo')\";>上传</label></div></td>");
        $.parser.parse('#logoTr');
        $("#logo").textbox("setValue",dizhi);
    }
    $("#type").combobox({
