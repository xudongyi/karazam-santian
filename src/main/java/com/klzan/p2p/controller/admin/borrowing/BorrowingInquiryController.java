...
public class BorrowingInquiryController extends BaseAdminController {
     ...
     * @return
     */
    @RequiresPermissions("borrowing:inquiry")
    @RequestMapping(value = "inquiry/{id}", method = RequestMethod.GET)
    public String verifyView(@PathVariable("id") Integer id, Model model) {

        //操作
        model.addAttribute("action", "inquiry");
        ...
        return template("borrowing/inquiry");
    }
    ...
    @ResponseBody
    public Result verifyData(BorrowingVo vo, BorrowingCheckState state, String opinion, HttpServletRequest request) {

        if (vo == null || vo.getId() == null) {
            logger.error("借款调查失败：借款ID不存在");
            return Result.error("借款ID不存在");
        }

        if (state == null) {
            logger.error("借款调查失败：参数错误");
