...
public final class SecrecyUtils {
    ...
    public static String toUsername(String username, String replaceChar) {
        ...
        // 创建格式化字符串
        // 获取并添加用户名第一个字符
        StringBuffer format = new StringBuffer(StringUtils.left(username, 0));
        // 添加替换字符
        format.append(DEFAULT_REPLACE_CHAR);
    ...
    public static String toName(String name, String replaceChar) {
        ...
        if(nameLength <= 3){
            format.append(REPLACE_CHAR_SEPARATOR);
            format.append(StringUtils.left(name, 1));
            ...
            format.append(StringUtils.isBlank(replaceChar) ? StringUtils.rightPad(DEFAULT_REPLACE_CHAR, nameLength - 1,
                    DEFAULT_REPLACE_CHAR) : replaceChar);
            format.append(StringUtils.right(name, 1));
        }
        return format.toString();
    }
    ...
    public static String toMobile(String mobile, String replaceChar) {
        ...
        // 添加替换字符
        format.append(REPLACE_CHAR_SEPARATOR);
        format.append(StringUtils.isBlank(replaceChar) ? StringUtils.rightPad(DEFAULT_REPLACE_CHAR, 6,
                DEFAULT_REPLACE_CHAR) : replaceChar);
        format.append(REPLACE_CHAR_SEPARATOR);
        // 获取并添加手机号码后4位字符
        format.append(StringUtils.substring(mobile, 9, 11));
...
//                format.append(StringUtils.rightPad(DEFAULT_REPLACE_CHAR, 4, DEFAULT_REPLACE_CHAR));
                format.append(REPLACE_CHAR_SEPARATOR);
                format.append(StringUtils.rightPad(DEFAULT_REPLACE_CHAR, 8, DEFAULT_REPLACE_CHAR));
                format.append(REPLACE_CHAR_SEPARATOR);
                format.append(StringUtils.substring(identity, 14, 15));
            }
            // 18位身份账号
            else {
//                format.append(StringUtils.rightPad(DEFAULT_REPLACE_CHAR, 4, DEFAULT_REPLACE_CHAR));
                format.append(REPLACE_CHAR_SEPARATOR);
                format.append(StringUtils.rightPad(DEFAULT_REPLACE_CHAR, 10, DEFAULT_REPLACE_CHAR));
                format.append(REPLACE_CHAR_SEPARATOR);
                format.append(StringUtils.substring(identity, 16, 18));
            }
        } else {
     ...
     * @return 格式化后的营业执照号
     */
    public static String toCorpLicenseNo(String identity, String replaceChar) {
        ...
        if (StringUtils.isBlank(replaceChar)) {
            format.append(REPLACE_CHAR_SEPARATOR);
            format.append(StringUtils.rightPad(DEFAULT_REPLACE_CHAR, 9, DEFAULT_REPLACE_CHAR));
            format.append(REPLACE_CHAR_SEPARATOR);
            format.append(StringUtils.substring(identity, identity.length()-2, identity.length()));
        } else {
            format.append(replaceChar);
        }
     ...
     * @return 格式化后的营业执照号
     */
    public static String toCorpName(String identity, String replaceChar) {
        ...
        if (StringUtils.length(identity) <= 6) {
            return identity;
        }

        // 创建格式化字符串
        StringBuffer format = new StringBuffer(StringUtils.left(identity, 2));
        String end = StringUtils.substring(identity, StringUtils.length(identity) - 4, StringUtils.length(identity));
        int recidualStrLen = StringUtils.length(identity) - 6;
        for (int i = 0; i < recidualStrLen; i++) {
            if (StringUtils.isBlank(replaceChar)) {
    ...
    public static String toBackCard(String backcard, String replaceChar) {
        ...
        if (identityLength < 12 || identityLength > 22) {
            return null;
        }

        // 创建格式化字符串
        // 获取并添加银行卡号前4位字符
        StringBuffer format = new StringBuffer(StringUtils.left(backcard, 4));
        // 添加替换字符
        if (StringUtils.isBlank(replaceChar)) {
            format.append(REPLACE_CHAR_SEPARATOR);
            format.append(StringUtils.rightPad(DEFAULT_REPLACE_CHAR, 4, DEFAULT_REPLACE_CHAR));
